                                      ...
                                      <!-- // 체크박스 -->
                                       <!-- 설명 -->
                                        <div class="description">
                                            <!-- 상품 이미지 -->
                                             <p class="prdImg">
                                                <a href="	/detail.html?product_no=2104&cate_no=44">
...
// 쿠키 데이터 디버깅 로그
console.log("Raw cookie data:", basketItems);

try {
    // 쿠키가 있으면 JSON 파싱
    let items = basketItems ? JSON.parse(basketItems) : [];

    // JSON 파싱 후 데이터 확인
    console.log("Parsed items:", items);

    // 2차원 배열 형태의 텍스트 생성
    let outputText = items.map(item => {
        // 디버깅용으로 각 필드 출력
        ...
        return `[상품 ID: ${item.id || ''}, 상품명: ${item.pname || ''}, 가격: ${item.price || ''}, 수량: ${item.quantity || ''}]`;
    }).join("\n");

    // 최종 생성된 outputText 확인
    console.log("Output text:", outputText);

    // 출력
    document.getElementById("cookie-output").textContent = outputText;
} catch (e) {
    console.error("JSON 파싱 에러:", e);
    document.getElementById("cookie-output").textContent = "쿠키를 불러오는 중 에러가 발생했습니다.";
}
