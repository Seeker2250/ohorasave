			...
			<div id="cate-wrap">
				...
				<div id="cate_list">
					...
					<ul class="menu_1ul" style="display: none">
						<li class="selected-menu-1" id="selected-menu-all"><a
							href="#">전체상품</a></li>
					...
					<ul class="menu_2ul">
						<li class="selected-menu-2" id="selected-menu-all"><a
							href="#">ALL</a></li>
			...
			<div id="item-list-wrap">
										...
										<div class="item-container">
											<dl>
												...
												<div class="item-info">
													...
													<div class="cart-in">
														<img src="../image/btn_list_cart.gif"
															data-pdtid="${pdt.pdt_id}" alt="" />
													</div>
												</div>
											</dl>
										</div>
					...
					<div class="popup-wrap" id="popup">
						<div class="popup">
							<div class="popup-head">
								<span class="head-title"><img
									src="/projectOhora/resources/images/cart_image/icon_cart_gray.png">
				...
				<script>
				    ...
				    $(document).ready(function() {
				    	   // 페이지 로드시 현재 장바구니 상태 출력
				    	   console.log("현재 모든 쿠키:", document.cookie);
				    	   showBasketItems();  // 현재 장바구니 상태 표시

				    	   $(document).on("click", "div.cart-in img", function(event) {
				    	       console.log("장바구니 버튼 클릭됨");
				    	       
				    	       let clickedPdtId = $(this).data("pdtid");
				    	       console.log("선택된 상품 ID:", clickedPdtId);
				    	       
				    	       // 상품 정보 가져오기
				    	       let $productInfo = $(this).closest('.item-container');
				    	       let price = $productInfo.find('.sale-price').text();//.replace(/[^0-9]/g, '')
				    	       let pname = $productInfo.find('.item-name').text();
				    	       
				    	       // 할인율과 이미지 URL 추가
				    	       let discountRate = $productInfo.find('.dcRate').text().replace('%', '');
				    	       discountRate = discountRate ? parseInt(discountRate) : 0;
				    	       
				    	       let imgUrl = $productInfo.find('.item-image img:first').prop('src');
				    	       ...
				    	       console.log("쿠키에서 읽어온 장바구니:", basketItems);

				    	       if (basketItems) {
				    	           try {
				    	               items = JSON.parse(basketItems);
				    	               console.log("파싱된 장바구니 아이템:", items);
				    	           } catch (e) {
				    	       ...
				    	       // 중복 체크
				    	       let existingItem = items.find(item => item.id === String(clickedPdtId));
				    	       if(existingItem) {
				    ...
				    function saveBasket(items) {
					    let basketJson = JSON.stringify(items);
					    console.log("저장할 장바구니 JSON:", basketJson);
					    
					    document.cookie = "basketItems=" + basketJson + "; path=/; max-age=86400";
					    console.log("저장 후 전체 쿠키:", document.cookie);
					    
					    // 저장 확인
					    let savedItems = getCookie("basketItems");
					    console.log("저장된 장바구니 확인:", savedItems);
					}
				    	// 장바구니 정보 출력 함수 수정
				    	function showBasketItems() {
				    	   console.log("=== 현재 장바구니 상태 ===");
				    	   let items = getCookie("basketItems");
				    	   
				    	   if(items) {
				    	       try {
				    	           let parsedItems = JSON.parse(items);
				    	           console.table(parsedItems);  // 테이블 형태로 예쁘게 출력
				    	           
				    	           parsedItems.forEach(item => {
				    	               console.log("=== 상품 상세 정보 ===");
				    	               console.log("상품ID:", item.id);
				    	               console.log("상품명:", item.pname);
				    	               console.log("가격:", parseInt(item.price).toLocaleString(), "원");
				    	               console.log("수량:", item.quantity);
				    	               console.log("할인율:", item.discountRate, "%");
				    	               console.log("이미지:", item.imgUrl);
				    	               console.log("---------------");
				    	           });
				    	           
				    	           let totalPrice = parsedItems.reduce((sum, item) => {
				    	               let price = parseInt(item.price);
				    	               let discount = item.discountRate > 0 ? price * (item.discountRate / 100) : 0;
				    	               return sum + ((price - discount) * item.quantity);
				    	           }, 0);
				    	               
				    	           console.log("총 상품 수:", parsedItems.length);
				    	           console.log("총 금액:", totalPrice.toLocaleString(), "원");
				    	           
				    	           return parsedItems;
				    	       } catch(e) {
				    	           console.error("장바구니 정보 확인 중 에러:", e);
				    	           return [];
				    	       }
				    	   } else {
				    	...
				    	function testBasket() {
				    	   ...
				    	   // 저장된 내용 확인
				    	   console.log("저장 직후 쿠키:", document.cookie);
				    	   showBasketItems();
				    	}
				    </script>
